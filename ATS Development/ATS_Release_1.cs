...
float I_TERM = 4F;
float D_TERM = 0.2F;
float INTEGRAL_DECAY = 0.85F; // Percent of integral to take to next PID cycle
...
void GyroTurn6(Vector3D TARGET, double GAIN, List<IMyGyro> Gyros, IMyShipController REF_RC, double ROLLANGLE,double MAXANGULARVELOCITY) {
    ...
    if (HAS_PID_LCD) {
        updatePIDLcd(ShipForwardAzimuth, ShipForwardElevation);
    }
...
void updatePIDLcd(double Azimuth, double Elevation) {
    String a = "";

    a += "Azimuth: " + Azimuth.ToString("0.00") + "\n";
    a += "Elevation: " + Elevation.ToString("0.00") + "\n";
